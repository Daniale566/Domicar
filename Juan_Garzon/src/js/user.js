    ...
    onGetAppointments((querySnapshot) => {
        ...
        querySnapshot.forEach(doc => {
            ...
            <details class="details" title="Click para ver detalles">
                ...
                <div>
                    <p>Número de Contacto:<br>
                    <span>${appointment.num}</span></p>
                    <p>Descripción/Comentarios:<br>
                    <span>${appointment.description}</span></p>
                </div>
            </details>
      ...
      const btnsDelete = appointmentsContainer.querySelectorAll('.btn-delete')
        ...
        btnsEdit.forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                appointmentForm['appoint_num'].value = Appointment.num
...
appointmentForm.addEventListener('submit', (e) => {
    ...
    const num = appointmentForm['appoint_num']
    const description = appointmentForm['appoint-desc']
    

    if (!editStatus){
        saveAppointment(tp_srvc.value, hr.value, dir.value, num.value, description.value)
